...
const multerUpload = multer({ dest: './public/upload/' });  // multer上传文件目录配置（从根目录开始算）
...
// 多文件上传
router.post('/files', multerUpload.array('file', 10), function (req, res, next) {
    if (req.files.length === 0) {
        ...
        for (let i = 0; i < req.files.length; i++) {
            ...
            console.log(file);
            fs.renameSync('./public/upload/' + file.filename, path + '/' + file.originalname);  // 可以修改文件名称
